...
class Data(object):
    def __init__(self, adj, edge_list, features, labels, train_mask, val_mask, test_mask):
        ...
        self.class_dim=int(torch.max(labels))+1
...
def load_data(self,dataset,norm=True):
    names=['x','y','tx','ty','allx','ally','graph']
    objects=[]
    for name in names:
        with open("planetoid/ind.{}.{}".format(dataset,name),'rb') as f:
    ...
    test_idx = parse_index_file("planetoid/ind.{}.test.index".format(dataset))
